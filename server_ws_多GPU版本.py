...
class FaceSwapServer:
    ...
    def processar_frames(self):
        ...
        while self.processamento_ativo:
            try:
                # 获取待处理帧（带序列号）
                sequence, frame = self.raw_frames.get(timeout=2.0)
                
                # 如果是第一次处理成功，打印一条信息
                if not first_processed:
                    print(f"线程 {thread_name} 开始处理第一个帧 {sequence}")
                    first_processed = True
                else:
                    print(f"线程 {thread_name} 开始处理帧 {sequence}")
                ...
                if self.frame_buffer.size() >= 30:
                    ...
                    if oldest_seq is not None and sequence > oldest_seq + 10:
                        print(f"清空缓冲区，当前序列号: {sequence}, 最旧序列号: {oldest_seq}")
                ...
                try:
                    ...
                    print(f"线程 {thread_name} 人脸交换完成")
                except Exception as e:
                    print(f"线程 {thread_name} 人脸交换错误: {e}")
                    continue

                # 人脸增强（使用多GPU）
                try:
                    new_face = face_enhancer.enhance_face(new_face)
                    print(f"线程 {thread_name} 人脸增强完成")
                except Exception as e:
                    print(f"线程 {thread_name} 人脸增强错误: {e}")
                    # 即使增强失败，我们也继续处理
                    pass
                ...
                print(f"线程 {thread_name} 帧 {sequence} 已放入缓冲区")
    ...
    def mostrar_estatisticas(self):
        ...
        while self.processamento_ativo:
            try:
                ...
                # 如果没有处理任何帧，减少打印频率
                if current_count == last_count and empty_stat_count < 10:
                    empty_stat_count += 1
                    if empty_stat_count % 3 == 0:  # 每15秒打印一次
                        print(f"[统计] 等待客户端发送帧... 队列大小: {self.raw_frames.qsize()}")
            ...
            try:
                ...
                if current_time - self.last_sent_time >= self.send_interval:
                    ...
                    if frame_b64 is not None and sequence != self.last_sent_sequence:
                        ...
                        # 如果没有帧可发送，减少打印频率
                        empty_send_count += 1
                        if empty_send_count % 100 == 0:  # 每100次循环打印一次
                            print("发送循环: 等待处理完成的帧...")
                            empty_send_count = 0
        ...
        try:
            async for mensagem in websocket:
                if mensagem.startswith('FRAME:'):
                    ...
                    print(f"收到客户端帧，序列号: {sequence}, 队列大小: {self.raw_frames.qsize()}")
                else:
