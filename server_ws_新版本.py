...
import numpy as np
import base64
import time  # 修复：导入整个time模块
from queue import Queue
import threading
import torch  # 添加torch用于显存监控
...
class FaceSwapServer:
    def __init__(self, source_image_path="photos/cr7.jpg", max_workers=10):
        ...
        self.start_time = time.time()  # 修复：使用 time.time()
    ...
    def processar_frames(self):
        ...
        while self.processamento_ativo:
            try:
                # 获取待处理帧（带超时）
                frame = self.raw_frames.get(timeout=1.0)
                
                t1 = time.time()  # 修复：使用 time.time()
                ...
                processing_time = time.time() - t1  # 修复：使用 time.time()
                ...
                if self.processed_count % 10 == 0:
                    print(f"线程 {thread_name} - 处理时间: {processing_time:.3f}s")
    ...
    def preparar_frames_para_envio(self):
        ...
        while self.processamento_ativo:
            try:
                frame = self.processed_frames.get(timeout=1.0)
    ...
    def mostrar_estatisticas(self):
        ...
        while self.processamento_ativo:
            try:
                time.sleep(5)  # 修复：现在 time 是模块，可以使用 sleep
                with self.stats_lock:
                    current_count = self.processed_count
                    elapsed_time = time.time() - self.start_time  # 修复：使用 time.time()
                    
                processed_since_last = current_count - last_count
                fps = processed_since_last / 5  # 过去5秒的FPS
                ...
                print(f"[队列] 原始帧: {self.raw_frames.qsize()}, 处理中: {self.processed_frames.qsize()}, 待发送: {self.frames_para_enviar.qsize()}")
                print(f"[显存] {gpu_info}")
                print("-" * 50)
    ...
    async def enviar_frames_processados(self):
        print("发送循环启动")
        last_sent_time = time.time()  # 修复：使用 time.time()
        sent_count = 0
        
        while True:
            try:
                current_time = time.time()  # 修复：使用 time.time()
