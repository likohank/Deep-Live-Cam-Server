...
def benchmark_worker(process_id, iterations, frame_data, device_id, result_queue):
    """压测工作进程"""
...
def run_multiprocess_benchmark(total_processes=10, iterations_per_process=5, frame_file="frame.dat"):
    ...
    for i in range(total_processes):
        ...
        process = mp.Process(
            ...
            args=(i, iterations_per_process, frame_data, device_id, result_queue)
        )
    ...
    for result in results:
        ...
        if times:
            ...
            print(f"进程 {process_id} (GPU{device_id}): 平均 {avg_time:.3f}s, 最小 {min_time:.3f}s, 最大 {max_time:.3f}s")
